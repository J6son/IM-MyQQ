/****************************************************************************
** Meta object code from reading C++ file 'video_read.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.6.2)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../MyQQ/RecordVideo/video_read.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'video_read.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.6.2. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_Video_Read_t {
    QByteArrayData data[10];
    char stringdata0[111];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_Video_Read_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_Video_Read_t qt_meta_stringdata_Video_Read = {
    {
QT_MOC_LITERAL(0, 0, 10), // "Video_Read"
QT_MOC_LITERAL(1, 11, 18), // "SIG_sendVideoFrame"
QT_MOC_LITERAL(2, 30, 0), // ""
QT_MOC_LITERAL(3, 31, 7), // "QImage&"
QT_MOC_LITERAL(4, 39, 3), // "img"
QT_MOC_LITERAL(5, 43, 18), // "slot_getVideoFrame"
QT_MOC_LITERAL(6, 62, 14), // "slot_openVideo"
QT_MOC_LITERAL(7, 77, 15), // "slot_closeVideo"
QT_MOC_LITERAL(8, 93, 12), // "slot_setMoji"
QT_MOC_LITERAL(9, 106, 4) // "type"

    },
    "Video_Read\0SIG_sendVideoFrame\0\0QImage&\0"
    "img\0slot_getVideoFrame\0slot_openVideo\0"
    "slot_closeVideo\0slot_setMoji\0type"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_Video_Read[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       6,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       1,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    1,   44,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       5,    0,   47,    2, 0x0a /* Public */,
       6,    0,   48,    2, 0x0a /* Public */,
       7,    0,   49,    2, 0x0a /* Public */,
       8,    1,   50,    2, 0x0a /* Public */,
       8,    0,   53,    2, 0x2a /* Public | MethodCloned */,

 // signals: parameters
    QMetaType::Void, 0x80000000 | 3,    4,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void, QMetaType::Int,    9,
    QMetaType::Void,

       0        // eod
};

void Video_Read::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Video_Read *_t = static_cast<Video_Read *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->SIG_sendVideoFrame((*reinterpret_cast< QImage(*)>(_a[1]))); break;
        case 1: _t->slot_getVideoFrame(); break;
        case 2: _t->slot_openVideo(); break;
        case 3: _t->slot_closeVideo(); break;
        case 4: _t->slot_setMoji((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 5: _t->slot_setMoji(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (Video_Read::*_t)(QImage & );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&Video_Read::SIG_sendVideoFrame)) {
                *result = 0;
                return;
            }
        }
    }
}

const QMetaObject Video_Read::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_Video_Read.data,
      qt_meta_data_Video_Read,  qt_static_metacall, Q_NULLPTR, Q_NULLPTR}
};


const QMetaObject *Video_Read::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *Video_Read::qt_metacast(const char *_clname)
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_Video_Read.stringdata0))
        return static_cast<void*>(const_cast< Video_Read*>(this));
    return QObject::qt_metacast(_clname);
}

int Video_Read::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 6)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 6;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 6)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 6;
    }
    return _id;
}

// SIGNAL 0
void Video_Read::SIG_sendVideoFrame(QImage & _t1)
{
    void *_a[] = { Q_NULLPTR, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}
QT_END_MOC_NAMESPACE
